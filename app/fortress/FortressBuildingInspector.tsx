...
export function BuildingInspector({
  ...
  return (
    <section className="relative overflow-hidden rounded-[22px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,20,31,0.62),rgba(6,8,14,0.82))] p-2.5 shadow-[0_16px_36px_rgba(0,0,0,0.22)] backdrop-blur-xl">
      ...
      <div className="relative z-[1] flex flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2.5">
          <FortressIcon name={meta.icon} size="lg" />
          <div>
            <div className="text-[10px] font-black uppercase tracking-[0.18em] text-[#f5d498]">{buildingShort(building.id, t)}</div>
            <div className="mt-0.5 text-lg font-black text-white">{label} {t("fortressScreen.buildings.level", { level })}</div>
          </div>
        </div>
      ...
      <p className="relative z-[1] mt-1 max-w-[46rem] text-[11px] leading-4 text-white/50">{buildingPerk(building.id, t)}</p>
      <div className="relative z-[1] mt-2 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
        ...
        <button
          className="frontline-motion-action frontline-feedback-upgrade rounded-[18px] border border-[#f8d57b]/28 bg-[linear-gradient(180deg,#fff0bc_0%,#f5c451_46%,#b96d1f_100%)] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#221102] shadow-[0_16px_32px_rgba(245,196,81,0.22)] transition disabled:opacity-40 disabled:hover:translate-y-0"
          disabled={!affordable}
          onClick={onUpgrade}
        >

...
import { BattleStatsPanel, BossSignaturePreview, EmptyCard, Panel, ResultMetric, RewardPreview } from "@/components/game/frontline/BattlePagePanels";
      ...
      <div className="mt-2 space-y-1 text-[10px] font-black uppercase tracking-[0.14em]">
        ...
        {(allyTraitInfo || enemyTraitInfo) ? (
          <div className="flex items-center justify-between gap-1 pt-1 text-[9px] text-white/56">
            <span className="text-cyan-100/82">{allyTraitInfo ?? " E}</span>
            <span className="opacity-60">vs</span>
            <span className="text-rose-100/82">{enemyTraitInfo ?? " E}</span>
          </div>
        ) : null}
      </div>
    </details>

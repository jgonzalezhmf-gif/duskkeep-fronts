...
type EffectPatch = Partial<HomeLandmarkEffectConfig>;

export type HomeEffectsQaEditorState = {
  selectedId: string | null;
  onSelect: (id: string) => void;
  onChange: (id: string, patch: EffectPatch) => void;
};
        ...
        return (
          <button
            ...
            className={cn(
              ...
              renderMode === "disabled" ? "border-rose-300/85 bg-rose-300/10" : effect.id === editor.selectedId ? "border-yellow-300 bg-yellow-300/12" : "border-sky-300/80 bg-sky-300/8",
            )}
            style={{
      ...
      <div className="grid gap-1 rounded border border-cyan-300/20 bg-black/45 px-2 py-1">
        <div className="flex items-center justify-between gap-2">
          <input
            ...
            step="0.01"
            value={normalized}
            onChange={(event) => onChange(clamp(Number(event.target.value) || 0, 0, 1))}
          />
          <input

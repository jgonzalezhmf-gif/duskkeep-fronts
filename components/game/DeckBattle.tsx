...
import { DeckBattleCommandPanel } from "@/components/game/deckbattle/DeckBattleCommandPanel";
import { DeckBattleHand } from "@/components/game/deckbattle/DeckBattleHand";
import {
  BattleContextPanel,
  LeaderPanel,
  LeaderPowerPanel,
      ...
      <div className="relative min-h-dvh px-2 py-2 md:px-4 md:py-3 lg:h-full lg:px-6">
        <div className="mx-auto flex min-h-[calc(100dvh-1rem)] max-w-[1440px] flex-col gap-2 md:min-h-[calc(100dvh-1.5rem)] md:gap-2.5 lg:h-full lg:min-h-0">
          <header className="grid shrink-0 grid-cols-2 gap-2 lg:grid-cols-[11.5rem_minmax(0,1fr)_11.5rem] lg:items-start">
            <DeckBattleCommandPanel
              round={tacticalState.round}
              activeSide={tacticalState.side}
              title={commandTitle}
              body={commandBody}
              focus={selectedPlay ? "Card" : focusedUnit ? "Unit" : "Board"}
              hint={commandHint}
              onClearFocus={() => clearSelections()}
            />
          ...
          <section className="rounded-[22px] bg-[linear-gradient(180deg,rgba(12,16,24,0.04),rgba(7,10,16,0.78))] p-2 shadow-[0_14px_24px_rgba(0,0,0,0.22)] backdrop-blur-md md:p-2.5">
            ...
            <div className="relative grid gap-2 md:grid-cols-[10rem_minmax(0,1fr)_10rem]">
              ...
              <DeckBattleHand
                hand={allySide.hand}
                selectedCardId={selectedPlay?.type === "card" ? selectedPlay.cardId : null}
                playableIds={playableIdSet}
                mana={allySide.mana}
                maxMana={allySide.maxMana}
                canPlay={tacticalState.side === "ally"}
                onCardClick={(cardId) => {
                  sfx.tap();
                  clearBoardFocus();
                  setSelectedPlay((current) =>
                    current?.type === "card" && current.cardId === cardId ? null : { type: "card", cardId },
                  );
                }}
              />

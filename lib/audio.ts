...
const THEME_MIX: Record<AudioThemeName, ThemeMixProfile> = {
...
function nextVariant(key: string, total: number) {
...
function readStoredMuted(fallback: boolean) {
  if (typeof window === "undefined") return fallback;
  try {
...
export const sfx = {
  ...
  move: () => {
    ...
    mgr.playFigure(figures[variant], { bus: "sfx", tempo: 142, duck: 0.06 });
  },
  attack: () => {
    mgr.vibrate(12);
    playSfx("attack", () => {
  ...
  defeat: () => {
    mgr.vibrate([24, 40, 80]);
    mgr.playStinger("defeat");
  },
  claim: () => {
    mgr.vibrate(18);
    playSfx("claim", () => {

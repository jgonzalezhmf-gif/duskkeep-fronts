...
async function main() {
  ...
  const selected = requestedIds.length ? plan.music.filter((item) => requestedIds.includes(item.id)) : plan.music;
  const missing = requestedIds.filter((id) => !selected.some((item) => item.id === id));

  if (missing.length) {
    throw new Error(`Unknown music id(s): ${missing.join(", ")}`);
  }
  ...
  if (!apiKey) {
    throw new Error("ELEVENLABS_API_KEY is missing. Set it in the session or .env.local before generating music.");
  }

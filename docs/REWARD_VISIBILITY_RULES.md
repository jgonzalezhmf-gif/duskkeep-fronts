...
Regla: si `claimed`, `cleared` o `firstClearTaken` ya es true, no se muestra como reward disponible.
...
- Daily Login streak.
- Future battle pass / reward track.
...
- Rewards de Arena por rival.
- Rewards base de Adventure al repetir nodo.
- Forecast de Fortress raid antes de resolver.
...
- `features/frontline/adventure.ts` centraliza el merge de rewards de Adventure para Frontline: precombat preview y victoria usan `getFrontlineAdventureRewardPreview(...)` / `getFrontlineAdventureVictoryRewards(...)`.
...
- Si es asset/reward opcional, usar manifest/fallback y no URLs especulativas.
...
- Si es reward result, mostrar solo lo realmente otorgado en esa accion.

## Riesgos A Vigilar

- No confundir `firstClearRewards` con rewards base repetibles.
- No ocultar Arena/Fortress previews si siguen siendo acciones repetibles.

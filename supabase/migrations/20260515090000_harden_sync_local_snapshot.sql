-- Duskkeep Fronts - hardening de importacion de snapshot local.
-- Mantiene el flujo de progreso invitado, pero deja de crear IDs arbitrarios
-- enviados por el cliente: heroes, cartas, loadout, garrison, nodos e
-- interactuables deben existir en catalogos/server state permitidos.
...
declare
  ...
  v_squad_non_null_count int := 0;
  v_squad_distinct_count int := 0;
  v_squad_owned_count int := 0;
  v_deck_non_null_count int := 0;
  v_deck_distinct_count int := 0;
  v_deck_owned_count int := 0;
begin
  ...
  if jsonb_typeof(p_snapshot -> 'heroes') = 'array' and jsonb_array_length(p_snapshot -> 'heroes') <= 64 then
    ...
    loop
      ...
      if v_key ~ v_id_pattern and exists (
        select 1
          from public.server_upgradeable_heroes h
          where h.hero_id = v_key
            and h.enabled = true
      ) then
  ...
  if jsonb_typeof(p_snapshot -> 'frontlineCardUnlocks') = 'object'
    ...
    loop
      if v_key ~ v_id_pattern
        ...
        and exists (
          select 1
            from public.server_upgradeable_frontline_cards c
            where c.card_id = v_key
              and c.enabled = true
        ) then
  ...
  if jsonb_typeof(v_loadout) = 'object'
    ...
    and coalesce(v_loadout ->> 'leaderId', '') in ('leader_aurora', 'leader_morrow', 'leader_elowen')
    and not exists (
      select 1
        from jsonb_array_elements(v_loadout -> 'squad') as squad_slot(value)
        where jsonb_typeof(squad_slot.value) not in ('string', 'null')
    )
    and not exists (
      select 1
        from jsonb_array_elements(v_loadout -> 'deck') as deck_slot(value)
        where jsonb_typeof(deck_slot.value) not in ('string', 'null')
    ) then

    with squad_ids as (
      select value #>> '{}' as hero_id
        from jsonb_array_elements(v_loadout -> 'squad')
    ...
    with deck_ids as (
      select value #>> '{}' as card_id
        from jsonb_array_elements(v_loadout -> 'deck')
        where jsonb_typeof(value) = 'string'
    )
    ...
    if v_squad_non_null_count = v_squad_distinct_count
      and v_squad_owned_count = v_squad_non_null_count
      and v_deck_non_null_count = v_deck_distinct_count
      and v_deck_owned_count = v_deck_non_null_count then
  ...
  if jsonb_typeof(v_frontline_fortress) = 'object'
    ...
    if jsonb_typeof(v_frontline_fortress -> 'garrison') = 'array'
      and jsonb_array_length(v_frontline_fortress -> 'garrison') = 3
      and not exists (
        select 1
          from jsonb_array_elements(v_frontline_fortress -> 'garrison') as garrison_slot(value)
          where jsonb_typeof(garrison_slot.value) not in ('string', 'null')
      ) then

      with garrison_ids as (
        select value #>> '{}' as hero_id
          from jsonb_array_elements(v_frontline_fortress -> 'garrison')
          where jsonb_typeof(value) = 'string'
      )
      select count(*), count(distinct hero_id)
        into v_squad_non_null_count, v_squad_distinct_count
      ...
      select count(*)
        ...
        join public.player_heroes h
          on h.profile_id = v_profile_id
          and h.hero_id = garrison_ids.hero_id
          and h.unlocked = true;

      if v_squad_non_null_count = v_squad_distinct_count
        and v_squad_owned_count = v_squad_non_null_count then
    ...
    loop
      ...
      if v_key ~ v_id_pattern then
        select chapter_id
          into v_chapter_id
          from (
            select node_id, chapter_id from public.server_adventure_battle_nodes where enabled = true
            union all
            select node_id, chapter_id from public.server_adventure_node_rewards where enabled = true
          ) nodes
          where nodes.node_id = v_key
          limit 1;
      end if;
  ...
  if jsonb_typeof(p_snapshot -> 'adventureMapClaims') = 'object'
    ...
    loop
      if v_key ~ v_id_pattern
        and jsonb_typeof(v_value -> 'claimed') = 'boolean'
        and (v_value ->> 'claimed')::boolean
        and exists (
          select 1
            from public.server_adventure_map_interactions i
            where i.interaction_id = v_key
              and i.enabled = true
        ) then

...
### Fixed
- Eliminado el bloque de opciones de confort del menu de configuracion para evitar que Reduced Motion, Visual Effects y Text Size degraden la experiencia visual del MVP.
- Ajustado el subtitulo del menu de opciones para describir solo las secciones disponibles: idioma, audio y cuenta.
...
### Fixed
- Pulido el Home para eliminar contadores duplicados superiores de Rewards/Pass, mantener un unico acceso temporizado inferior y hacer que el contador de Pass avance en tiempo real.
- Ajustado el CTA principal de Adventure para ampliar su area clicable y alinear mejor el icono dentro del bloque de mision principal.

## [0.38.1] - 2026-06-30

### Fixed
- Corregido el estado de Daily Tribute para que una recompensa diaria autoritativa no vuelva visualmente a `Day 1` si el refresco posterior del snapshot llega sin claims diarios recientes.
...
### Fixed
- Ajustado el panel de mision/interaccion del mapa Adventure para acoplarse arriba cuando el nodo seleccionado esta en la zona baja del mapa, evitando tapar la primera mision y cofres inferiores.
...
### Fixed
- Corregido el doble procesamiento del retorno OAuth de Supabase: el cliente ya no auto-detecta sesiones en la URL porque la app gestiona explicitamente `code`/`access_token`/`refresh_token`.
- Evitada una carrera que podia provocar `401 Unauthorized` contra `/auth/v1/user` tras Google OAuth al consumir el hash de autenticacion desde dos rutas distintas.
...
### Changed
- Extraido el render del nucleo/castillo de Fortress Defense a `app/fortress/FortressDefenseCastleKeep.tsx`.
- Reducido `app/fortress/FortressDefenseBattlefieldUnits.tsx` de 371 a 304 lineas sin cambiar posicionamiento, reglas de oleadas, dano, cooldowns, audio ni rewards.
...
### Changed
- Migrados los iconos runtime de combate a WebP desde `GAME_ICON_ASSET_MANIFEST.combat`, conservando dimensiones y transparencia.
- Guardado backup externo de los PNG sustituidos en `C:\Users\Usuario\DuskkeepAssetBackups\2026-05-27_0.37.14_combat_icons`.
- Actualizada la documentacion local de iconos para reflejar WebP como formato runtime de combate.
- Reducido el presupuesto de `public/assets` de `42.31 MB` a `41.97 MB`, sin referencias huerfanas en `audit:asset-refs`.
...
### Fixed
- Endurecida la clasificacion de links de setup de cuenta: solo `guestUpgrade=confirm` permite sincronizar snapshot local, mientras links genericos `type=email_change` cargan snapshot servidor.
...
### Fixed
- Resueltas las vulnerabilidades moderadas de dependencias transitivas fijando `ws` en `8.20.1` bajo Supabase Realtime y `brace-expansion` en `5.0.6` bajo TypeScript ESLint.
...
### Docs
- Anadida `docs/SCREEN_UX_CHECKLIST.md` con criterios de aceptacion, riesgos y estado UX por pantalla para demo y release candidate.
- Enlazada la checklist desde el indice documental, calidad/release y estado de proyecto.
...
### Security
- Anhadido guard de Fetch Metadata al proxy autoritativo.
- Requests de navegador con `Sec-Fetch-Site: cross-site` se rechazan antes del rate limit y antes de parsear el body.
- Las llamadas sin `Sec-Fetch-Site` siguen permitidas para smokes, tooling y clientes no navegador.

### Docs
- Documentada la defensa Fetch Metadata en los contratos autoritativos y roadmap de seguridad.

### Tested
- `npm.cmd test -- tests/server.authoritativeRequestGuards.test.ts tests/server.authoritativeRpcProxy.test.ts`
- `npm.cmd run typecheck`
- `npm.cmd run check`
- `npm.cmd test`
- `npm.cmd run build`
...
### Security
- Endurecida la extraccion del header `Authorization` en el proxy autoritativo.
- Los tokens Bearer con espacios internos, caracteres de control o caracteres no seguros se rechazan antes de preparar el cliente Supabase.
- Supabase sigue siendo la autoridad de autenticacion real; el guard solo filtra cabeceras anormales antes del RPC.

### Docs
- Documentado el limite defensivo de formato para `Authorization: Bearer`.
...
### Security
- Anhadidas cabeceras HTTP globales desde Next para endurecer navegador: CSP, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy` y `Permissions-Policy`.
- La CSP bloquea `object-src`, `base-uri` y `frame-ancestors`, permite conexiones al origen Supabase configurado y no permite `unsafe-eval` en produccion.
- `unsafe-eval` queda limitado a desarrollo para compatibilidad con tooling de Next.

### Docs
- Documentado el alcance del hardening de cabeceras y el riesgo residual de `unsafe-inline` hasta poder migrar a nonces/hashes.

### Tested
- `npm.cmd test -- tests/server.securityHeaders.test.ts`
- `npm.cmd run typecheck`
- `npm.cmd run check`
- `npm.cmd test`
- `npm.cmd run build`
